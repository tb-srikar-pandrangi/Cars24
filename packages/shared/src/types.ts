...
export type AgentEvent =
  | { type: 'metrics_ready'; payload: CampaignMetrics[] }
  | { type: 'funnel_health'; payload: FunnelHealth[] }
  | { type: 'diagnosis_ready'; payload: DiagnosisResult[] }
  | { type: 'alert'; payload: DiagnosisResult }
  | { type: 'creative_scored'; payload: CreativeScore }
  | {
  ...
  | { type: 'allocation_decided'; payload: AllocationDecision[] }
  | { type: 'allocation_applied'; payload: { campaignId: string; platform: string; result: string } }
  | { type: 'ingest_error'; payload: { source: string; error: string; stale: boolean } }
  | {
      type: 'competitor_alert';
      payload: { keyword: string; competitor: string; impressionShareLost: number };
    };
